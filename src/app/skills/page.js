...
import { 
  ...
  Layers, ArrowRight, ShieldCheck, Cpu, Database, Sparkles
} from 'lucide-react';
...
export default function SkillsPage() {
  return (
    <div style={{ position: 'relative', padding: '4.5rem 1.5rem 6.5rem 1.5rem', maxWidth: '1280px', margin: '0 auto' }}>
      ...
      {/* AMBIENT GOLD GLOWS */}
      <div className="ambient-glow" style={{ top: '6%', right: '8%', width: '450px', height: '450px', background: 'rgba(212, 175, 55, 0.12)' }} />
      <div className="ambient-glow" style={{ top: '40%', left: '5%', width: '480px', height: '480px', background: 'rgba(245, 215, 127, 0.08)' }} />
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '2.5rem', marginBottom: '5.5rem' }}>
        {skillCategories.map((cat) => (
          ...
          >
            ...
            <p style={{ color: '#a1a1aa', fontSize: '0.94rem', marginBottom: '1.75rem' }}>
              {cat.desc}
            </p>
            ...
            <div style={{
              ...
              {cat.skills.map((s, idx) => (
                ...
                >
                  ...
                  <p style={{ fontSize: '0.84rem', color: '#a1a1aa', lineHeight: 1.5, margin: 0 }}>
                    {s.detail}
                  </p>
                </div>
              ))}
            ...
            <div key={idx} className="glass-card" style={{ padding: '2rem 1.8rem', display: 'flex', flexDirection: 'column' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
                <span className="formula-pill" style={{ fontSize: '0.95rem' }}>
                  <Terminal size={15} color="#f5d77f" />
                  {f.formula}
                </span>
                <span style={{ fontSize: '0.75rem', color: '#71717a', fontWeight: 700, textTransform: 'uppercase' }}>
                  {f.category}
                </span>
              </div>

              <p style={{ fontSize: '0.88rem', color: '#e4e4e7', lineHeight: 1.6, marginBottom: '1.25rem', flexGrow: 1 }}>
                {f.description}
              </p>
      ...
      <div className="glass-card" style={{ padding: '3rem 2.5rem', background: 'rgba(16, 18, 25, 0.92)', marginBottom: '5rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '1.75rem' }}>
          <div>
            <div style={{ color: '#f5d77f', fontWeight: 700, fontSize: '1rem', marginBottom: '0.35rem' }}>
